...
class Dashboard
{
     ...
     * @param  object  $seller
     */
    public function getTodayStats($seller): array
    {
        return [
            'total_sales'  => $this->saleReporting->getTodaySalesProgress($seller),
            'total_orders' => $this->saleReporting->getTodayOrdersProgress($seller),
    ...
    public function getRecentOrders($seller)
    {
        foreach (['pending', 'processing', 'completed', 'refunded'] as $status) {
            $orders[$status] = $this->getOrders($seller, $status);
        }
