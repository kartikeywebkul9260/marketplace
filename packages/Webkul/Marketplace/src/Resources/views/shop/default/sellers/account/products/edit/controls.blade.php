@switch($attribute->type)
    @case('text')
        ...
        >
            <input
                ...
                class="flex w-full min-h-10 py-2.5 px-3 mb-3 border-2 rounded-md text-sm text-gray-600 transition-all hover:border-gray-400 focus:border-gray-400"
    ...
    @case('price')
        <div class="relative">
            <span class="absolute ltr:left-4 rtl:right-4 top-[40%] -translate-y-[50%] text-gray-500 {{ $attribute->code == 'price' ? 'text-xl' : '' }}">
            ...
            <x-marketplace::shop.form.control-group.control
                ...
                :label="$attribute->admin_name"
                value="{{ old($attribute->code) ?: $product[$attribute->code] }}"
                :class="'ltr:pl-7 rtl:pr-7' . ($attribute->code == 'price' ? 'py-2 bg-gray-50 text-xl font-bold' : '')"
            >
            </x-marketplace::shop.form.control-group.control>
        </div>
    @break
        ...
        <div class="flex gap-2.5">
                ...
                >
                    @if ($attribute->type == 'image')
                        <img
                            ...
                            class="w-[45px] h-[45px] border rounded overflow-hidden hover:border-gray-400"
                        />
                    @else
            ...
            >
                <input
                    ...
                    id="{{ $attribute->code }}"
                    :class="[errors['{{ $attribute->code }}'] ? 'border border-red-600 hover:border-red-600' : '']"
                    class="flex w-full min-h-10 py-2 px-3 border rounded-md text-sm text-gray-600 transition-all hover:border-gray-400 focus:border-gray-400"
                    @change="handleChange"
                    @blur="handleBlur"
                >
            </v-field>
